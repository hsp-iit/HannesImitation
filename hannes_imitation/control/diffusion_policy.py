...
class HannesDiffusionPolicyController:
    ...
    def __init__(self, hannes, policy, control_frequency, observation_horizon, action_horizon, frames_list, hannes_states_list):
        self.hannes = hannes
        self.policy = policy
    ...
    def get_observation(self):
        ...
        while len(obs_frames_raw) != self.observation_horizon:
            ...
            obs_frames_raw = self.frames_list[-self.observation_horizon:] # read the latest obs_horizon frames
            obs_frames_raw = np.array([frame['rgb'] for frame in obs_frames_raw]) # extract rgb key
        ...
        obs_frames = [cv2.rotate(frame, cv2.ROTATE_180) for frame in obs_frames] # rotate eye-in-hand frames
    ...
    def predict(self, obs_dict):
        ...
        action_trajectory = action_trajectory.reshape(self.action_horizon, -1) # remove batch dimension (Ta, Da)
    ...
    def step(self, action_trajectory):
        ...
        for action in action_trajectory:
            ...
            self.hannes.move_wristPS(int(action[2]))
